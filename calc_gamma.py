...
def calgamma(airfoil,mid_panel,length_panel,theta,sine, cosine, rhs, aoa ):
    ...
    for i in range (0,m):
        for j in range (0,m):
            if i == j:
                #kutta condition
                cn1 = np.zeros(shape=[m,m]); cn1[i,j] = -1
                cn2 = np.zeros(shape=[m,m]); cn2[i,j] = 1
                ct1 = np.zeros(shape=[m,m]); ct1[i,j] = np.pi/2
                ct2 = np.zeros(shape=[m,m]); ct2[i,j] = np.pi/2
            else:
    ...
    gamma = solve(an,rhs)
    v = np.zeros(shape=[m]) ; cp = np.zeros(shape=[m]) #initialize v and cp
    for i in range[0,m]:
